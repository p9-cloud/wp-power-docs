...
| **K** AI 驗收 | `detect_smoke` 檢查 diff 有無動到 `js/src/`、`inc/templates/`、`inc/classes/` → 建置前端（`pnpm run build`）→ Playwright 裝 chromium（`--with-deps`）→ 安裝 CJK 字型 → `run_ai_acceptance`（agent=`wp-workflows:browser-tester`） |
...
| Marketplace | `https://github.com/j7-dev/wp-workflows.git`（提供 clarifier / planner / tdd-coordinator / browser-tester agents） |
