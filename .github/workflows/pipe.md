...
| output | 意義 |
|--------|------|
| `branch_name` / `issue_num` | 本輪 `issue/{N}` 分支與 issue 編號（fallback: `issue/{N}-*` → `claude/issue-{N}-*`） |
...
| 段 | 核心動作 |
|----|---------|
| **A** 前置 | eyes reaction → checkout → `resolve_branch`（找或建 `issue/{N}`，fallback `issue/{N}-*` → `claude/issue-{N}-*`）→ HTTPS → `save_sha` |
...
| 段 | 核心動作 |
|----|---------|
| **H** 環境 | checkout(branch_name) → Node 20 / pnpm / composer → 建 uploads → wp-env start（3 次重試，delay 15/45/90s，unhealthy 容器 restart） |
| **I** PHPUnit 3 循環 | `test_cycle_1` 失敗 → `claude_fix_1` → `test_cycle_2` 失敗 → `claude_fix_2` → `test_cycle_3`（final，無修復）。所有步驟 `continue-on-error: true`，fix 走 `anthropics/claude-code-action@v1`；測試命令：`npx wp-env run tests-cli --env-cwd=wp-content/plugins/power-docs vendor/bin/phpunit --testdox --colors=never` |
| **J** 彙整 | `final_result` parse PHPUnit summary（`OK (...)` 或 `Tests: ...`）→ 發測試結果留言 |
| **K** AI 驗收 | `detect_smoke` 檢查 diff 有無動到 `js/src/`、`inc/templates/`、`inc/classes/` → 建置前端（`pnpm run build`）→ Playwright 裝 chromium（`--with-deps`）→ 安裝 CJK 字型 → `run_ai_acceptance`（agent=`zenbu-powers:browser-tester`） |
...
| Marketplace | `https://github.com/zenbuapps/zenbu-powers.git`（提供 clarifier / planner / tdd-coordinator / browser-tester agents） |
...
1. **無 LC Bypass 段**：本 plugin 的 `plugin.php` 不使用 Powerhouse `'capability'` 行的授權機制（與 Power Course 不同），故 pipe 中不套用 LC bypass。
2. **`.wp-env.json` 無 mappings**：外掛直接透過 `plugins: ["."]` 載入，容器內路徑固定為 `wp-content/plugins/power-docs`，`--env-cwd` 需與此對齊。
3. **port 配置**：`port: 8895`（開發）/ `testsPort: 8893`（測試），AI 驗收使用 `http://localhost:8893`。
4. **前端建置**：僅 `pnpm run build`（無 `build:wp` 第二段，Vite 配置已統一）。
5. **SPA 管理路由**：`#/docs`、`#/users`、`#/doc-access`、`#/media-library`、`#/bunny-media-library`。
6. **前台模板**：三種（doc-landing / doc-detail / doc-search）由 `single-pd_doc.php` 分派。
7. **AI 驗收路徑偵測**：`js/src/`、`inc/templates/`、`inc/classes/`（無 `inc/assets/`）。

---

## 六、Gotchas

1. **Claude fix prompt 寫死在 workflow**（I 段兩處約 60 行）：未來可搬到 `.github/prompts/claude-fix.md`。
2. **AI 驗收 prompt 寫死 `http://localhost:8893`**：與 `.wp-env.json` `testsPort` 耦合，改 port 時需同步。
3. **`npm test:phpunit` 已在 package.json**：若要本地跑，用 `pnpm run test:phpunit`（對應 `--env-cwd=wp-content/plugins/power-docs`）。
...
- [ ] `--env-cwd=wp-content/plugins/power-docs` 路徑與 `.wp-env.json` 一致？
